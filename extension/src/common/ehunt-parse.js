...
(function (root, factory) {
  ...
  function panelRoot(doc) {
    // Delegates to findPanelRoot so the scoping also works when EHunt mounted
    // its panel in a shadow root, which document.querySelector cannot see into.
    return findPanelRoot(doc) || doc;
  }
  ...
  function parsePanel(doc) {
    ...
    // exactly like one in the light DOM, and every label lookup stays scoped to
    // the panel rather than the whole Etsy page.
    const root = findPanelRoot(doc);
    if (!root) return null;
    const { tags, tagVolumes } = readTags(root);
    const { stats, growth, cells } = readStats(root);
